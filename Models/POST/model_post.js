...
exports.handleQueryProduct = (request, callback) => {
    const id = uuidv4()
    const name = request.payload.name
    const image = request.payload.image
    const price = request.payload.price
    const colors = request.payload.colors
    const condition = request.payload.condition
    const description = request.payload.description

    const queryStatment = "INSERT INTO `products`(`id`, `name`, `image`, `price`, `colors`, `condition_string`, `description`, `created_at`, `is_active`) VALUES (?,?,?,?,?,?,?,NOW(),1)"
    connection_mysql.connection.query(queryStatment, [id, name, image, price, colors, condition, description], (errorDB, resultsDB, fieldsDB) => {
        if (errorDB) {
            throw errorDB
